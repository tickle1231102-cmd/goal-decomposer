...
async function getRecentGoals(userId: string) {
...
export async function HomeDashboard({ user }: HomeDashboardProps) {
  const recentGoals = await getRecentGoals(user.id);
    ...
    <AppShell user={user}>
      <section className="mx-auto mb-10 max-w-2xl text-center">
        ...
        <p className="mt-4 text-base leading-7 text-muted-foreground text-pretty">
          장기 목표를 연·월·주·일 단위로 자동 분해하고, 오늘의 To-Do부터 바로
          실행할 수 있습니다.
        </p>
      </section>
